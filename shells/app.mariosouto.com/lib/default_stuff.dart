// ignore_for_file: constant_identifier_names

import 'package:flutter/material.dart';

class AppBaseData {
  static const String totalVideos = "143";
  static const String totalShorts = "50";
  static const String totalCursos = "10";
}

class AppBaseTheme {
  static const themeBackgroundWhite = Color(0xFFffffff);
  static const themeBackgroundGrey001 = Color(0xFFf9fafb);
  static const themeBackgroundColor = Color(0xFFf3f4f6);
  static const themeGrey001 = Color(0xFF9ca3af);
  static const themeGrey002 = Color(0xFF111827);
  static const themeGrey003 = Color(0xFF4b5563);
  static const themeGrey004 = Color(0xFF24292e);
  static const themeBlueIndigo001 = Color(0xFF4f46e5);
  static const themeBlueOrangeVivid001 = Color(0xFFff7f50);
  static const scale_x2 = 8.0;
  static const scale_x4 = 16.0;
  static const scale_x5 = 20.0;
  static const scale_x6 = 24.0;
  static const scale_x8 = 32.0;
  static const scale_x10 = 40.0;
}
